...
int add_client(SOCKET sock, int sockType)	//client=Publisher?
{ 
	int i;
	switch (sockType)
	{
...
unsigned int fill_set(fd_set *fds) //was ist das?
{
...
void newMCAddr(char *lastMCAddr) 
{
	...
	addrBuff.u.Word[7] = addrBuff.u.Word[7] + i;//.... erhöhe um i, Achtung network byte order!
...
void sendUDPMsg(struct genericMsg* annMsg, char *mcGroupAddr, char *port)
{
	...
	//....
	char *portip[2] = { mcGroupAddr, port };
	sendto(ServSock[UDP][0], annMsg->pad1, annMsg->header.msgLength, 0, portip, annMsg->header.msgLength);
};
...
int exitServer()
{
	//...
	int i;
	for (i = 0; i < numSocks[TCP]; i++)
		close(ServSock[TCP][i]);

	close(ServSock[UDP][0]);
